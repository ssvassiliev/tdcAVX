...
inline double vec_dot(double *,double *);
inline void vec_diff(double *, double *, double *);
inline void vec_scale(double *, double, double *);
inline void Line(int);
...
void ReadGaussianCube(int FNum,int Det)
{
 ...
 double min_Q=10.0,max_Q=.0;
  ...
  for(i=0;i<cube[Det].NumAtoms;i++) {
  	fscanf(GaussianCube,"%d%lf%lf%lf%lf", &ATM, &junk, &X, &Y, &Z);
	cube[Det].AtomicXYZ_Num[i] = _mm_set_ps((float)ATM,(float)Z,(float)Y,(float)X);
  }
  ...
  i=0;
  double q_charge;
  float tmp_vec[4][4] __attribute__ ((aligned (16)));
  long v_element_count = 0;
  long v_count = 0;

  cube[Det].rX = (__m128*)calloc(0,sizeof(__m128));
  cube[Det].rY = (__m128*)calloc(0,sizeof(__m128));
  cube[Det].rZ = (__m128*)calloc(0,sizeof(__m128));
  cube[Det].rQ = (__m128*)calloc(0,sizeof(__m128));
  ...
  for (ix=0;ix<cube[Det].MX;ix++) 
      ...
      for (iz=0;iz<cube[Det].MZ;iz++) 
	{
	  ...
	  if(abs_Q > Qcutoff) {
	    ...
	    charged++;
	    v_element_count++;
	    if ( v_element_count == 4 ) {
	      cube[Det].rX = (__m128*)realloc(cube[Det].rX,(v_count+1)*sizeof(__m128));
	      cube[Det].rY = (__m128*)realloc(cube[Det].rY,(v_count+1)*sizeof(__m128));
	      cube[Det].rZ = (__m128*)realloc(cube[Det].rZ,(v_count+1)*sizeof(__m128));
	      cube[Det].rQ = (__m128*)realloc(cube[Det].rQ,(v_count+1)*sizeof(__m128));
	      cube[Det].rX[v_count] = _mm_set_ps(tmp_vec[0][3],tmp_vec[0][2],tmp_vec[0][1],tmp_vec[0][0]); 
	      cube[Det].rY[v_count] = _mm_set_ps(tmp_vec[1][3],tmp_vec[1][2],tmp_vec[1][1],tmp_vec[1][0]);
	      cube[Det].rZ[v_count] = _mm_set_ps(tmp_vec[2][3],tmp_vec[2][2],tmp_vec[2][1],tmp_vec[2][0]);
	      cube[Det].rQ[v_count] = _mm_set_ps(tmp_vec[3][3],tmp_vec[3][2],tmp_vec[3][1],tmp_vec[3][0]);
	      v_count++;
	      v_element_count=0;
  ...
  ix=0;
  float extract[4] __attribute__ ((aligned (16)));
  ...
  while((fabs(diffsum)>cut) && (ix < maxit))
	...
	alpha*=0.1;
      ...
      for (i=0;i<v_count;i++) 
	{
	  _mm_store_ps(extract,cube[Det].rQ[i]);
	  for(j=0; j<4; j++) 
	    {
	  ...
	  extract[0] = extract[1] = extract[2] = extract[3] = 0.0f;
	}
	  ...
	  cube[Det].rQ[i] = _mm_set_ps(extract[3],extract[2],extract[1],extract[0]);
      }
      correct = cube[Det].sum/cube[Det].NumOfVoxels;
      prevdiffsum = diffsum;
      diffsum = fabs(cube[Det].sum) - fabs(prevsum);
  ...
  fflush(stderr);
  float x_extract[4] __attribute__ ((aligned (16)));
  float y_extract[4] __attribute__ ((aligned (16)));
  float z_extract[4] __attribute__ ((aligned (16)));
  // Find the center of 'charge' 
  vec_scale(cube[Det].center,0.0,cube[Det].center); 
  for (i=0;i<v_count;i++) {
      _mm_store_ps(extract,cube[Det].rQ[i]);
      _mm_store_ps(x_extract,cube[Det].rX[i]);
      _mm_store_ps(y_extract,cube[Det].rY[i]);
      _mm_store_ps(z_extract,cube[Det].rZ[i]);
      for(j=0; j<4; j++) {
...
double CalcCoulombCoupling(int C1, int C2, int cache_size)
{
 ...
 __m128 diff[4];
 __m128 r_vec, result, vcovlps, vcps;
 __m128 R_cutoff_check, R_CUTOFF;
 float tmp_add[4][4] __attribute__ ((aligned (16)));\
 float x_extract[4]  __attribute__ ((aligned (16)));\
 __m128 tmpQ[4], tmpX[4], tmpY[4], tmpZ[4];
 ...
 _mm_store_ps(x_extract,cube[C2].rX[cube[C2].v_count-1]);
 for(i=cube[C2].last_v_element_count; i<4; i++) 
   x_extract[i] = 1.0f;
 cube[C2].rX[cube[C2].v_count-1] = _mm_set_ps(x_extract[3],x_extract[2],x_extract[1],x_extract[0]);
      ...
      fprintf(stderr,"Block %li Thread: %i\n",h,id);fflush(stderr);
        for(i=id; i<cube[C1].v_count;i+=nthreads)
        {
           ...
           for(m=0; m<4; m++)
           {
              ...
              {
                 diff[0] = _mm_sub_ps(tmpX[m],cube[C2].rX[j]);
                 diff[1] = _mm_sub_ps(tmpY[m],cube[C2].rY[j]);
                 diff[2] = _mm_sub_ps(tmpZ[m],cube[C2].rZ[j]);
  
                 diff[0] = _mm_mul_ps(diff[0],diff[0]);
                 diff[1] = _mm_mul_ps(diff[1],diff[1]);
                 diff[2] = _mm_mul_ps(diff[2],diff[2]);
 
                 r_vec = _mm_add_ps(diff[0],diff[1]);
                 r_vec = _mm_add_ps(r_vec,diff[2]);
                 r_vec = _mm_rsqrt_ps(r_vec);
   
                 result = _mm_mul_ps(tmpQ[m],cube[C2].rQ[j]);
                 result = _mm_mul_ps(result,r_vec);
                 vcps = _mm_add_ps(vcps,result);       // Accumulate coupling between all voxels
                 R_cutoff_check = _mm_cmple_ps(r_vec,R_CUTOFF);
                 result = _mm_and_ps(R_cutoff_check,result);
                 vcovlps = _mm_add_ps(vcovlps,result); // Accumulate coupling excluding overlapped
              }
              _mm_store_ps(tmp_add[1],vcps);
              _mm_store_ps(tmp_add[0],vcovlps);
              VC += tmp_add[0][0] + tmp_add[0][1] + tmp_add[0][2] + tmp_add[0][3];
              VC_OVL += tmp_add[1][0] + tmp_add[1][1] + tmp_add[1][2] + tmp_add[1][3];
           }
	}      
     }	
...
inline double vec_dot(double *a, double *b)
{
  return(a[0]*b[0]+a[1]*b[1]+a[2]*b[2]);
}


inline void vec_scale(double *a, double c, double *ac)
{
int i;
  for(i=0;i<3;i++)
    ac[i]=c*a[i];
...
inline void vec_diff(double *a, double *b, double *diff)
{
  int i;
  for(i=0;i<3;i++)
    diff[i]=a[i]-b[i];
